...
// // комментарий
    ...
    // metka: while (true) {
    //     team = prompt ("За какую команду болеете?").toUpperCase();
    //     switch (team) {
    //         case "СПАРТАК": console.log("Фу! Свинья!"); break metka;
    //         case "ДИНАМО": console.log("Фу! Мусор!"); break metka;
    //         case "ЛОКОМОТИВ": console.log("Фу! Лох!"); break metka;
    //         case "ЦСКА": console.log("Фу! Конявый!"); break metka;
    //         case "ЗЕНИТ": console.log("Фу! Бомж!"); break metka;
    //         default: console.log("Нет такой команды!");         
    //     }
    ...
    function allTheNumbers(n1, n2) {
        ...
        if (typeof(n1) == "number" && typeof(n2) == "number") {
            ...
            console.log("The function allTheNumbers requires numbers as arguments. But the arguments are: "+n1+", "+n2);
        }
    }
    ...
    function allTheFibonacciNumbers(n1, n2) {
        ...
        if (typeof(n1) == "number" && typeof(n2) == "number") {
            ...
            console.log("The function allTheFibonacciNumbers requires numbers as arguments. But the arguments are: "+n1+", "+n2);
        }
    }
    ...
    function allTheNechetNumbers(n1, n2) {
        ...
        if (typeof(n1) == "number" && typeof(n2) == "number") {
            ...
            console.log("The function allTheNechetNumbers requires numbers as arguments. But the arguments are: "+n1+", "+n2);
        }
    }
    ...
    function allTheMod3Numbers(n1, n2) {
        ...
        if (typeof(n1) == "number" && typeof(n2) == "number") {
            ...
            console.log("The function allTheMod3Numbers requires numbers as arguments. But the arguments are: "+n1+", "+n2);
        }
    }
    ...
    function rect (width,height) {
        var str = "<div style=\"width: "+width+"px; height: "+height+"px; background: red;\"></div>";
        document.write(str);
    }
    function circle (r) {
        var str = "<div style=\"width: "+r*2+"px; height: "+r*2+"px; border-radius: 50%; background: black;\"></div>";
        document.write(str);
    }
    ...
    function add3dFace(x,y,z,width,height,color,transform,container,text) {
    ...
    function createParallelepiped(x, y, width, height, depth, id) {
        ...
        upButton.style.top = (y - Math.sqrt(height*height + width*width + depth*depth)*0.5 -25) + "px";
        //div.style.top = upButton.style.top;
        div.style.height = Math.sqrt(height*height + width*width + depth*depth) + "px";
        ...
        downButton.style.top = (y + Math.sqrt(height*height + width*width + depth*depth)*0.5 +5) + "px";
        ...
        rightButton.style.left = (x + Math.sqrt(height*height + width*width + depth*depth)*0.5 + 0) + "px";
        ...
        leftButton.style.left = (x - Math.sqrt(height*height + width*width + depth*depth)*0.5 - 20) + "px";
        ...
        var div1 = add3dFace(-width/2, -height/2, 0, width, height,
                            "orange", "translate3d(0px, 0px,+"+depth/2+"px)", div1a,"enter text 1");
        div1.addEventListener("click", textEditor);
        div1a.style.transform = "rotateY(180deg)";
        
        var div2 = add3dFace(-width/2, -height/2, 0, width, height,
                            "orange", "translate3d(0px, 0px,+"+depth/2+"px)", cntYrot,"enter text 2");
        div2.addEventListener("click", textEditor);
                            var div3 = add3dFace(-(width+depth)/2, -height/2, 0, depth, height,
                            "green", "rotateY(-90deg)", cntYrot, "enter text 3");
        div3.addEventListener("click", textEditor);
        var div4 = add3dFace((width-depth)/2, -height/2, 0, depth, height,
                            "green", "rotateY(90deg)", cntYrot, "enter text 4");
        div4.addEventListener("click", textEditor);
        var div5 = add3dFace(-width/2, -(height+depth)/2, 0, width, depth,
                            "yellow", "rotateX(90deg)", cntYrot, "enter text 5");
        div5.addEventListener("click", textEditor);
        var div6 = add3dFace(-width/2, (height-depth)/2, 0, width, depth,
                            "yellow", "rotateX(-90deg)", cntYrot, "enter text 6");
        ...
        switch (re.exec(e.target.id)[2]) {
            case "xPlus":
                interval = setInterval(function(){
                    ...
                    document.getElementById(id+"Xrot").style.transform = "rotateX("+angleX[id]+"deg)";
                },25);
                break;        
            case "xMinus":
                interval = setInterval(function(){
            ...
            case "yPlus":
                interval = setInterval(function(){
                    ...
                    document.getElementById(id+"Yrot").style.transform = "rotateY("+angleY[id]+"deg)";
                },25);
                break;
            case "yMinus":
                interval = setInterval(function(){
    ...
    function textEditor(e) {
        if (e.target.nodeName == "H2") {
            e.target.childNodes[0].nodeValue = prompt("Старый текст: "+e.target.childNodes[0].nodeValue+"\nВведите новый текст: ");
        }
    ...
    function Parallelepiped(x, y, width, height, z, id) {
        ...
        this.height = height;
        this.id = id;
        this.div = createParallelepiped(this.x, this.y, this.width, this.height, this.depth, this.id);
        this.show = function() {
            try {
